...
function createReportPdf(r,resultTone,projectName){
  ...
  function drawMetricCards(items){
    ...
    ensure(cardH*rowCount+gap*Math.max(0,rowCount-1)+10);
    ...
    y+=cardH*rowCount+gap*Math.max(0,rowCount-1)+8;
  }
  ...
  if(state.vertical==='brand'&&Number.isFinite(r.engagements)){
    reportMetrics.push([t('engagementsMetric'),count(r.engagements)],[t('costPerEngagement'),money(r.costPerEngagement)]);
    if(Number.isFinite(r.errReach)) reportMetrics.push([t('errReach'),pct(r.errReach)]);
    if(Number.isFinite(r.erImpressions)) reportMetrics.push([t('erImpressions'),pct(r.erImpressions)]);
    if(Number.isFinite(r.erAudience)) reportMetrics.push([t('erAudience'),pct(r.erAudience)]);
  }
